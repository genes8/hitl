...
- API: added GET /api/v1/applications/{id} (optional tenant scope via `tenant_id` query param).
- API: application detail now includes latest scoring_result when available.
- API: application detail now includes queue_entry + decision_history (when present).
