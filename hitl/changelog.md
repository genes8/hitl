...
- API: extend GET /api/v1/applications/{id} detail payload with latest queue_info + decision_history.
- API: extend GET /api/v1/applications/{id} detail payload with similar_cases when present (+ tests).
- API: add DELETE /api/v1/applications/{id} endpoint to cancel (soft delete) pending applications (+ tests).
